...
export default function TripPage() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>((location.state as any)?.tab ?? 'tickets')
  ...
  const visibleTabs = TRIP_TABS.filter(t => {
    if (t.key === 'expenses') return trip.settings.showExpenses
    if (t.key === 'car') return trip.settings.showCar
    return true
  })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <div className="flex-1 overflow-auto" style={{ paddingBottom: 'calc(80px + env(safe-area-inset-bottom))' }}>
        ...
        {activeTab === 'itinerary' && <ItineraryTab trip={trip} initialView={(location.state as any)?.view} />}
