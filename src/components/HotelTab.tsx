    ...
    <div className="pb-6">
      ...
      <div className="px-4 space-y-5">
        {hotelTickets.map((ticket, idx) => {
          ...
          const isActive = checkInDate && checkOutDate ? checkInDate <= now && checkOutDate >= now : false
          const statusLabel = isPast ? 'Past stay' : isActive ? 'Now staying' : 'Upcoming'
          const statusOpacity = isPast ? 0.45 : 1
          ...
          return (
            <div key={ticket.id}>
                ...
                <div className="relative" style={{ height: 180 }}>
                  ...
                  <div className="absolute top-3.5 left-4">
                    <span className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full"
                      style={{ background: '#000812', color: `rgba(255,255,255,${statusOpacity})` }}>
                      ● {statusLabel}
                    </span>
                  </div>
                  {data.bookingRef && (
              ...
              {/* Next stay */}
              {nextData && (() => {
                const nextCheckInDate = tryParseDate(nextData.checkIn?.match(/[A-Z]\w+\s+\d{1,2},?\s*\d{4}/i)?.[0] ?? '')
